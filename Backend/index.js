...
mongoose.connect("mongodb://127.0.0.1/REACTAPP")
...
// Configure upload middleware with file filter
const upload = multer({
    storage: storage,
...
//creating endpontfor user login
...
//creating endpoint for popular in ring section
app.get('/popularinrings',async (req,res)=>{
    let products =await Product.find({category:"Ring"});
    let popular_in_rings=products.slice(0,4);
    console.log("Popular in ring fetched");
    res.send(popular_in_rings);
})
